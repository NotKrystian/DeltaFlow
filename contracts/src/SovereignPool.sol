...
contract SovereignPool is ISovereignPool, ReentrancyGuard {
        ...
        returns (uint256 amountInUsed, uint256 amountOut)
    {
        ...
        if (address(swapCache.swapFeeModule) != address(0)) {
            swapFeeModuleData = swapCache.swapFeeModule
                .getSwapFeeInBips(
        ...
        ALMLiquidityQuote memory liquidityQuote = ISovereignALM(alm)
            .getLiquidityQuote(
        ...
        if (liquidityQuote.amountInFilled != swapCache.amountInWithoutFee) {
            effectiveFee = Math.mulDiv(
                liquidityQuote.amountInFilled, swapFeeModuleData.feeInBips, _MAX_SWAP_FEE_BIPS, Math.Rounding.Ceil
            );
        ...
        returns (uint256 feePoolManager0Received, uint256 feePoolManager1Received)
    {
        ...
        if (sovereignVault != address(this)) {
            ...
            ISovereignVaultMinimal(sovereignVault)
                .claimPoolManagerFees(feePoolManager0Received, feePoolManager1Received);
    ...
    function _handleTokenInTransfersOnSwap(
        ...
        if (isSwapCallback) {
            ISovereignPoolSwapCallback(msg.sender)
                .sovereignPoolSwapCallback(address(token), amountInUsed, _swapCallbackContext);
        } else {
