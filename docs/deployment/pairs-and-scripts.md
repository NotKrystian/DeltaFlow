...
| `MIN_PERP_HEDGE_SZ` | Optional; if set **> 0**, calls **`SovereignVault.setMinPerpHedgeSz`**. HL **`sz`** units: until the hedge bucket **≥** this minimum, **`tokenOut` is escrowed** in the vault (queued payouts); when the threshold is met, **one IOC** runs for the full bucket and **all** queued recipients are paid in that tx. **`0`** = **immediate `tokenOut` + one IOC per swap** (no escrow). See [Current implementation](../architecture/current-implementation.md#on-chain-per-swap-perp-hedge-and-batch-queue). |
